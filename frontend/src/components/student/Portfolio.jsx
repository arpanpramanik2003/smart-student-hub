        ...
        <div className="px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mt-4 sm:mt-6">
            {[
              ...
              { value: Object.keys(activityGroups).length, label: "Categories", icon: "📂" },
              { value: activities.length > 0 ? Math.round(stats?.totalCredits / activities.length * 10) / 10 : 0, label: "Avg Credits", icon: "📊" }
            ].map((stat, index) => (
              <div
                key={index}
