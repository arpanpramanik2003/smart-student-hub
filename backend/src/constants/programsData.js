...
const getCategoryValue = (categoryKeyOrValue) => {
...
const getSpecializations = (category, degree) => {
...
// Helper function to validate program selection (accepts both KEY and VALUE)
const validateProgramSelection = (category, degree, specialization) => {
  const programs = getProgramsByCategory(category);
  const program = programs.find(p => p.degree === degree);
  
  if (!program) {
    return { valid: false, message: 'Invalid program selected' };
  }
  
  // Check if specialization is required and provided
  if (program.specializations.length > 0 && !specialization) {
    return { valid: false, message: 'Specialization is required for this program' };
  }
  
  // Check if provided specialization is valid
